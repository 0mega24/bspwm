...
*bspc* scratchpad toggle ['PROFILE'|last]|hide ['PROFILE']|hide_focused|close ['PROFILE']|cycle|list-profiles
...
FLAG := hidden|sticky|private|locked|scratch|marked|urgent
...
[!](hidden|sticky|private|locked|scratch|marked|urgent)::
...
scratch::
	Scratchpad window: floating area sized by *scratch_width_ratio* and *scratch_height_ratio* relative to the tileable region (centered), not movable or resizable via pointer or *node --move* / *node --resize*. Enabling *scratch* also enables *sticky*. New windows are inserted relative to the same public tile targets as for *private* nodes (not as a split against a focused scratchpad). When the focused monitor changes, all scratchpad windows are moved to the focused desktop of that monitor. EWMH close-window requests are ignored.
...
*-g*, *--flag* hidden|sticky|private|locked|scratch|marked[=on|off]::
...
*-a*, *--add* (<class_name>|\*)[:(<instance_name>|\*)[:(<name>|\*)]] [*-o*|*--one-shot*] [monitor=MONITOR_SEL|desktop=DESKTOP_SEL|node=NODE_SEL] [state=STATE] [layer=LAYER] [honor_size_hints=(true|false|tiled|floating)] [split_dir=DIR] [split_ratio=RATIO] [(hidden|sticky|private|locked|scratch|marked|center|follow|manage|focus|border)=(on|off)] [rectangle=WxH+X+Y]::
...
toggle ['PROFILE']::
	Without *PROFILE*, if the focused window is a visible *scratch* node, hide it and return. Otherwise, if *scratchpad_default* is set, find a *scratch* node whose *instance_name* or *class_name* equals that name; otherwise find the first *scratch* node (any class/instance). Toggle *hidden*, and focus when showing. If none exists, run *scratchpad_spawn_command* when set.
	With *PROFILE*, only consider *scratch* nodes whose *instance_name* or *class_name* equals *PROFILE* (exact match). If none exists, run the command from *scratchpad_profile_PROFILE* (see settings). Profile names use letters, digits, *\_* and *-*.
	When *showing* a scratch window, every other visible scratch window is hidden first (at most one scratch visible).

toggle last::
	Same toggle behaviour as above, but always targets the most recently *shown* scratch window (updated when a scratch becomes visible after *toggle*, *cycle*, or initial map). Fails if there is no recorded scratch yet or that node no longer exists. The literal second argument *last* is reserved here and is not a profile name.

hide ['PROFILE']::
	Without *PROFILE*, hide every visible *scratch* node. With *PROFILE*, hide only *scratch* nodes matching that name (same rule as *toggle*).

hide_focused::
	If the focused window is a *scratch* node, hide it. Otherwise no-op. Useful for key bindings without affecting other scratch windows.

close ['PROFILE']::
	With *PROFILE*, send a close request (*WM_DELETE_WINDOW* or kill) to every *scratch* node whose *instance_name* or *class_name* matches *PROFILE*. Without *PROFILE*, close only the focused window if it is *scratch*; otherwise fail.

cycle::
	No-op if there are no *scratch* windows. Otherwise, among all *scratch* windows (including hidden), in stable tree order (monitors, desktops, leaves): the *current* pad is the focused window if it is scratch; else the first visible scratch in that order if any; else the cycle starts *before* the first entry (so *next* is the first scratch). Then hide every scratch window except *next* (wrapping), unhide *next* if it was hidden, and focus it. At most one scratch window remains visible.

list-profiles::
	Print each configured *scratchpad_profile_* as *NAME*=*COMMAND* (one line per profile). Profile arguments to *toggle* and *hide* are matched after trimming leading and trailing ASCII whitespace.
...
'scratch_width_ratio'::
'scratch_height_ratio'::
	Fraction of the tileable layout rectangle used for *scratch* floating windows (width and height respectively). Each value must be greater than *0* and at most *1*. Defaults to *0.5*.

'scratch_autohide'::
	When *true*, hide scratchpad windows automatically: after focus moves to another window (debounced by *scratch_autohide_blur_delay_ms*), and immediately after desktop/monitor switches (debounced by *scratch_autohide_desktop_delay_ms*). Replaces an external *subscribe* autohide script. Defaults to *false*.

'scratch_autohide_blur_delay_ms'::
	Milliseconds to wait before hiding non-focused scratch windows after a *node_focus* event. Range *0*–*10000*. Defaults to *250*.

'scratch_autohide_desktop_delay_ms'::
	Milliseconds to wait before hiding all visible scratch windows after *desktop_focus*, *desktop_activate*, or *monitor_focus*. Range *0*–*10000*. Defaults to *50*.
...
'scratchpad_spawn_command'::
	Shell command run when *scratchpad toggle* is sent with no *PROFILE* and no matching *scratch* window exists yet. Empty by default.

'scratchpad_default'::
	If non-empty, *scratchpad toggle* with no *PROFILE* uses this profile name only when searching for a *scratch* window to toggle (after the focused-visible-scratch hide step). Spawn still uses *scratchpad_spawn_command*, not *scratchpad_profile_*. Set to the same *NAME* as your default terminal (e.g. *scratchpad*) so unprofiled toggle targets that pad instead of the first scratch in tree order. Empty by default (legacy: first scratch window of any kind).

'scratchpad_profile_'*NAME*::
	For each profile *NAME* (letters, digits, underscore, hyphen), the shell command run when *scratchpad toggle NAME* finds no matching *scratch* window. Set with *bspc config scratchpad_profile_term '...'*, read with *bspc config scratchpad_profile_term*. Set to an empty string to remove a profile. Up to 16 profiles. Use exactly one underscore between *profile_* and *NAME* (*scratchpad_profile_ffscratch*); an extra underscore (*scratchpad_profile__ffscratch*) is accepted and normalized to *NAME* without the leading underscore.
...
'node_flag <monitor_id> <desktop_id> <node_id> hidden|sticky|private|locked|scratch|marked|urgent on|off'::
...
'G(S?P?L?R?M?)'::
	Active flags of the focused node of a focused desktop (*S* sticky, *P* private, *L* locked, *R* scratch, *M* marked).
